...
void testApp::setup(){	
    ...
    score.loadFont("LetterGothicStd.otf", 14);
...
void testApp::update(){
    ...
    if (bDead ==false) {
        // bike
        float speed = ofMap(ofxAccelerometer.getForce().x, -1, 1, -0.04, 0.04);
        ...
        for (int i=0; i<rocks.size(); i++) {
            ...
            if (rectAx1 < rectBx2 && rectAx2 > rectBx1 &&
                ...
                speed = 0;
                bikePct= ofNoise(bikePct-0.001,bikePct+0.001);
            }
        }
        ...
        if (bike.pos.x<150||bike.pos.x>ofGetWidth()-150) {
            bover = false;
            bDead = false;
        }
...
//--------------------------------------------------------------
void testApp::draw(){
//    ofSetColor(191, 107, 4);
//    ofRect(0, 0, ofGetWidth(), ofGetHeight()/2);
//    ofSetColor(217, 47, 88);
//    ofRect(0, ofGetWidth()/2, ofGetWidth(), ofGetHeight());
//    ofSetColor(0, 220, 255);
    ...
    ofPushMatrix();
        ofTranslate(ofGetWidth()/2+100, ofGetHeight()/2-30);
        ofRotateZ(180);
        ofSetColor(255, 255, 255);
        score.drawString("Butterfly: "+ofToString(Score), 0, 0);
    ofPopMatrix();
