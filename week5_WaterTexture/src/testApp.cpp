...
void testApp::setup(){	
    ...
    for (int x = 0; x < width; x++){
        for(int y = 0; y < height; y++){
            particle myParticle;
            myParticle.setInitialCondition(ofMap(x,0, width,0, ofGetWidth()),ofMap(y,0,height,0,ofGetHeight()),0,0);
...
//        particles[i].bounceOffWalls();
    ...
    VF.addNoise(0.1,0.00001,1,true);
	VF.fadeField(0.99f);
...
//	VF.addCounterClockwiseCircle((float)x, (float)y, 100, 0.1f);
	prevMouseX = x;
	prevMouseY = y;
}
